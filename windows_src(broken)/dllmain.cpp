...
typedef bool (__fastcall* MultiplayerCheck_t)(long long* a1, long long* a2);
MultiplayerCheck_t o_MultiplayerCheck;
MultiplayerCheck_t MultiplayerCheck_hook;
...
long long __fastcall hk_Execute(uint8_t* pThis, long long a2, long long* a3)
{
    ...
    *(uint8_t*)(pThis + 25) = false;
    ...
    long long o_Execute_result = o_Execute(pThis, a2, a3);

    bExecutingCommand = false;

    return o_Execute_result;
}

bool __fastcall hk_MultiplayerCheck(long long* a1, long long* a2)
{
    //fputs("[N-Phenethyl-4-piperidinone] MultiplayerCheck called.\n", stdout);

    if (bExecutingCommand)
        return true;

    return o_MultiplayerCheck(a1, a2);
}
...
void InputThread(HMODULE hModule)
{
    while (true)
    {
        if (GetAsyncKeyState(VK_INSERT) & 1)
        {
            ...
            MH_RemoveHook(MultiplayerCheck_hook);
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD  ul_reason_for_call, LPVOID lpReserved)
{
    if (ul_reason_for_call == DLL_PROCESS_ATTACH)
    {
        ...
        MultiplayerCheck_hook = MultiplayerCheck_t(FindPattern(const_cast<char*>("eu4.exe"), const_cast<char*>(""), const_cast<char*>("xxxx?xxxxxxxx??xxxxxxxxxxxx??xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx????xxxxxxxxxxxxxxxx?")));
        ...
        printf("[N-Phenethyl-4-piperidinone] MultiplayerCheck %p\n", MultiplayerCheck_hook);
        ...
        MH_CreateHook(MultiplayerCheck_hook, &hk_MultiplayerCheck, (LPVOID*)&o_MultiplayerCheck);
        MH_EnableHook(MultiplayerCheck_hook);
